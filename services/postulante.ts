...
export const buscarPostulante = async () => {
	...
	const postulanteService = await $api.postulante.getPostulante(profileStore.profileData?.data?.documenIdentida  ?? '', {lazy: true,})
	...
	postulanteService.data.value?.data.length && postulanteStore.setPostulante(postulanteService.data.value?.data[0]);
}
...
export const getPostulante = async () => {
	...
	try{
	   ...
	   if(e.message == 'nodata'){
		...
		const registrarPostulante = await $api.postulante.registrarPostulante(request, {lazy: true});
