...
const selectedMethod = reactive({ section: '', title: '', url: '' });
...
const redirectPayment = () => {
	...
	if (window.dataLayer) {
		window.dataLayer.push({
			event: 'Click/Boton-RealizarPago',
...
const activatePaymentItem = (index: number) => {
	...
	if (index === 99) {
		buttonTextContinue.value = 'Continuar';
		selectedMethod.section = 'Otros';
		selectedMethod.title = 'Otros medios de pago';
		paymenyCheckTermsDisabled.value = false;
	} else {
		buttonTextContinue.value = 'Ir a la web del banco';
		selectedMethod.section = 'Bancos';
		selectedMethod.title = props.listPaymentMethods[index].nombreMedioDePago;
		selectedMethod.url = props.listPaymentMethods[index].url;
...
<template>
	<div class="content-popup">
		<div ref="target" class="popup-schedule">
			...
			<div class="card-content">
				<div>
					<span
						class="bg-dark_100 text-white font-grotesk py-1 px-2 font-semibold text-md"
				...
				<div class="flex items-center gap-3 mt-4 ms-1">
					<input
						type="checkbox"
						class="accent-primary scale-125"
				...
				<div class="flex mt-3">
					<button
						class="w-full rounded-md gap-1 text-sm font-extrabold bg-primary py-2.5 disabled:bg-[#FFE2BE] disabled:text-[#4F6168]"
						:disabled="
...
.content-popup {
	@apply fixed top-0 h-screen w-full bg-black bg-opacity-30 flex items-center justify-center z-[105] left-[50%] translate-x-[-50%] overflow-y-auto;
}

.popup-schedule {
	@apply relative bg-white lg:fixed z-40 lg:w-[550px] lg:rounded-[8px] lg:cursor-auto;
}

.close-schedule-popup {
	@apply text-[#000000] text-[16px] absolute right-4 top-4 leading-[1] font-[600] hover:text-primary;
}

.card-content {
	@apply px-[24px] pt-[8px] pb-[20px];
}

li.items-methods {
	@apply flex items-center border-2 border-[#C2D1D9] rounded-md py-2.5 px-3 my-2 gap-3;
}
